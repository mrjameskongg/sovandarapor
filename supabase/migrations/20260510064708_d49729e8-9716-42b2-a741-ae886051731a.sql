...
create policy "published posts readable by all" on public.posts for select using (status = 'published' or public.has_role(auth.uid(), 'admin'));
...
create policy "admins upload post-images" on storage.objects for insert with check (bucket_id = 'post-images' and public.has_role(auth.uid(), 'admin'));
create policy "admins update post-images" on storage.objects for update using (bucket_id = 'post-images' and public.has_role(auth.uid(), 'admin'));
create policy "admins delete post-images" on storage.objects for delete using (bucket_id = 'post-images' and public.has_role(auth.uid(), 'admin'));
